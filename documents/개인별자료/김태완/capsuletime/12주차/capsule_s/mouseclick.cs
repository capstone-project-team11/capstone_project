...
public class mouseclick : MonoBehaviour
{
    ...
    {
        //카메라 오브젝트 선언
        ar_camera = GameObject.Find("First Person Camera").gameObject;
    }
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity))
            {
                ...
                if (hit.transform.gameObject)
                {
                    ...
                    now_capsule.transform.LookAt(ar_camera.transform);//캡슐 클릭시 카메라 방향으로 회전(수정 필요)
    ...
    void pop_active()
    {
        //n_popup, c_popup은 각각 팝업창의 name과 content를 의미
        var n_popup = GameObject.Find("Canvas").transform.Find("popup").transform.Find("name").gameObject;
        var c_popup = GameObject.Find("Canvas").transform.Find("popup").transform.Find("content").gameObject;
        
        //popup script의 함수를 호출하여 현재 인식된 gameobject의 이름을 전달
        n_popup.GetComponent<popup>().insert_n(hit.transform.gameObject.name);
        c_popup.GetComponent<popup>().insert_c(hit.transform.gameObject.name);
